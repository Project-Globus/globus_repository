...
public class PGDB {
	...
	// Sends a Leave Group request to the Database.
	// Returns a string of query statuses. 
	public static String leaveGroup(String[] userInfo, String grpID) {
		for (int i = 0; i < userInfo.length; ++i)
			userInfo[i] = validate(userInfo[i]);
		...
		try {
			...
			if (!rs.first()){
				...
				status = sendMessage("DELETE FROM globus.GroupMembers WHERE group_id = '" + grpID +  "', user_id='" + userInfo[2] + "'");
			}
		} catch (SQLException e1) {
			status += e1.getMessage();
		}
	...
	// Returns a list of all messages from one week prior to the given timestamp
	// [0][x] - Query Statuses
	// [x][0] - Message ID
	// [x][1] - Group ID
	// [x][2] - Message Type	
	// [x][3] - Message Contents
	// [x][4] - Message Timestamp
	...
	// Insert a new message into the database for specified group ID. Returns query status.
	...
	// Returns a list of all group members in a given group
	// [0][x] - Query Statuses
	// [x][0] - User ID
	// [x][1] - User Name
	// [x][2] - User Email	
	...
	// Returns a list of all events in a given group
	// [0][x] - Query Statuses
	// [x][0] - Event ID
	// [x][1] - Event Name
	// [x][2] - Event Description
	// [x][3] - Event Time start
	// [x][4] - Event Time end
	// [x][5] - Event Calendar ID
	public static ArrayList<ArrayList<String>> getEvents(String[] userInfo, String grpID) {
		for (int i = 0; i < userInfo.length; ++i)
			userInfo[i] = validate(userInfo[i]);
		...
		try {
			...
			if (!rs.first()){
				...
				rs = sendQuery("SELECT event_id,name,description,time_start,time_end,cal_id FROM globus.Events, globus.Calendar WHERE Events.cal_id = SELECT cal_id FROM globus.Calendar WHERE Calendar.group_id = '" + grpID + "'");
				if (!rs.first()){
						result.get(0).add("ERROR RETRIEVING MEMBERS");
						return result;
				} else {
	...
	// Adds a new event to a calendar for a group
	// Returns the query status
	public static String addEvent(String[] userInfo, String calID, String start, String end, String name, String desc) {
		for (int i = 0; i < userInfo.length; ++i)
			userInfo[i] = validate(userInfo[i]);
		
		String response = "";
		
		calID = validate(calID);
		...
		try {
			ResultSet rs = sendQuery("SELECT group_name FROM globus.GroupMembers, globus.Calendar WHERE GroupMembers.group_id = SELECT group_id WHERE cal_id = '" + calID + "' AND GroupMembers.user_id ='" + userInfo[2] + "'");
			if (!rs.first()){
				response = "Member does not belong to group.";
				return response;
			} else {
				response = sendMessage("INSERT INTO globus.Events (cal_id,time_start,time_end,name,description) VALUES ('" + 
						calID + "','" + 
						start + "','" + 
						end + "','" +
						name + "','" + 
	...
	// Adds a new attendee to a given event
	// Returns the query status
	public static String addAttendee(String[] userInfo, String cal_id, String e_id, String u_id, String perm) {
		for (int i = 0; i < userInfo.length; ++i)
			userInfo[i] = validate(userInfo[i]);
		
		String response = "";
		
		cal_id = validate(cal_id);
		e_id = validate(e_id);
		u_id = validate(u_id);
		perm = validate(perm);
		
		try {
			ResultSet rs = sendQuery("SELECT group_name FROM globus.GroupMembers, globus.Calendar WHERE GroupMembers.group_id = SELECT group_id WHERE cal_id = '" + cal_id + "' AND GroupMembers.user_id ='" + userInfo[2] + "'");
			if (!rs.first()){
				response = "Member does not belong to group.";
				return response;
			} else {
				response = sendMessage("INSERT INTO globus.EventMembers (cal_id,event_id,user_id,permissions) VALUES ('" + 
						cal_id + "','" + 
						e_id + "','" + 
						u_id + "','" +
						perm + "')");
	...
	// Removes a specified attendee from a given event
	// Returns the query status
	public static String removeAttendee(String[] userInfo, String cal_id, String e_id, String u_id, String perm) {
		for (int i = 0; i < userInfo.length; ++i)
			userInfo[i] = validate(userInfo[i]);
		...
		try {
			...
			if (!rs.first()){
				...
				response = sendMessage("DELETE FROM globus.EventMembers WHERE cal_id = '" + cal_id + "', event_id='" + e_id + "', user_id='" + u_id + "'");
			}
		} catch (SQLException e1) {
			response = e1.getMessage();
		}
	...
	// [x][3] - User Email	
	// [x][4] - User Biography
	public static ArrayList<ArrayList<String>> getAttendees(String[] userInfo, String cal_id, String e_id) {
		for (int i = 0; i < userInfo.length; ++i)
			userInfo[i] = validate(userInfo[i]);
		...
		try {
			String grpID = "";
			ResultSet rs = sendQuery("SELECT group_id FROM globus.Calendar WHERE cal_id = '" + cal_id + "'");
			if (!rs.first()){
				result.get(0).add("Calendar does not belong to group.");
				return result;
			} else {
				grpID = rs.getString(1);
				rs = sendQuery("SELECT group_name FROM globus.GroupMembers WHERE group_id = '" + grpID + "' AND user_id ='" + userInfo[2] + "'");
				if (!rs.first()){
					result.get(0).add("Member does not belong to group.");
					return result;
				} else {
					rs = sendQuery("SELECT user_id,name, FROM globus.GroupMembers, globus.Groups, globus.Calendar WHERE Events.cal_id = SELECT cal_id FROM globus.Calendar WHERE Calendar.group_id = '" + grpID + "'");
					if (!rs.first()){
							result.get(0).add("ERROR RETRIEVING MEMBERS");
							return result;
					} else {
	...
	// Allows the group creator to update the group description
	// Returns the query status
	...
	// Allows the group creator to update the group name
	// Returns the query status
	...
	// Allows the group creator to update the group password
	// Returns the query status
	...
	// Allows the group creator to update the group Google Account Information
	// Returns the query status
	...
	// Allows a user to update their biography
	// Returns the query status
	...
	// Allows a user to update their 'last_updated' timestamp; manual
	// Returns the query status
