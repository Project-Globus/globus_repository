...
public class PGDB {
	...
	public static ArrayList<ArrayList<String>> getMembership(String[] userInfo) {
		...
		try {
			ResultSet rs = sendQuery("SELECT group_name,group_id,password,description,creator,google_username,google_password FROM globus.Groups, globus.GroupMembers WHERE Groups.group_id=GroupMembers.group_id and GroupMembers.user_id = '" + userInfo[2] + "'");
			if (!rs.first()){
	...
	public static ArrayList<ArrayList<String>> getMessages(String[] userInfo, String grpID, String timestamp) {
		...
		try {
			...
			if (!rs.first()){
				...
				ResultSet rs = sendQuery("SELECT message_id,group_id,type,contents,timestamp FROM globus.Messages WHERE group_id = " + grpID + " and timestamp>"+timestamp-(86400*7)+ " ORDER BY timestamp ASC");
				if (!rs.first()){
	...
	public static ArrayList<ArrayList<String>> getGroupMembers(String[] userInfo, String grpID) {
		...
		try {
			...
			if (!rs.first()){
				...
				ResultSet rs = sendQuery("SELECT user_id,name,email,biography FROM globus.Users, globus.GroupMembers WHERE GroupMembers.group_id = '" + grpID + "'");
				if (!rs.first()){
